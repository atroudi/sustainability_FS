...
def import_crop(crop_demand, external_water_requirement, crop_inventory, country_facts, month, cost, env):
    ...
    for country in list(external_water_requirement_row.index):
        # if not (":" in country):
        country_dict.update({country:0}) # initialise

    tmp_import_cum = 0

    for country in list(external_water_requirement_row.index):
        row = country_facts[country_facts['Countries'] == country]
        # I changed the capacity by the demand so that the maximum we can import from a country is a percent of our demand, this also can
        # force imports from more than one country, i guess it s the case in reality, they import from many countries not only one

        # tmp_import = row['Capacity'].values[0] * row['Percent_Export'].values[0] * (0.05 * np.random.rand() + 0.05)
        tmp_import = row['Capacity'].values[0] * row['Percent_Export'].values[0]

        if (tmp_import + crop_inventory) >= crop_demand:
            import_amount = crop_demand - crop_inventory
        else:
            import_amount = tmp_import

        # store import
        country_dict.update({country:import_amount}) # update
        
        print("Amount imported from %s is %f" % (country, import_amount)) # logging import info
        
        cost += import_amount * row['Unit_Price'].values[0]
  
        # CO2 footprint for air transport is 0.85 kg CO2/km 0
        env += 0.85 * row['Distance'].values[0]

        # keep track of cumulative amount imported to return from this function
        tmp_import_cum += import_amount

        crop_inventory += import_amount 
        
        # i changed it to = instead of > so the maximum imported is the demand
    ...
    return tmp_import_cum, crop_inventory, cost, env, country_dict

# external_water_requirement is checked against local_water_requirement, and a decision is made to either: 
# (1) only grow; (2) only import; (3) both grow and import; or (4) do nothing
def make_decision(crop_demand, local_water_requirement, external_water_requirement, crop_inventory, country_facts, month):
    cost = 0; env = 0; tmp_import = 0; tmp_grow = 0
    import_dict = dict()
    ...
    if (crop_inventory < crop_demand):
        ...
        if (local_water_requirement < min_cwr): # this is for 'grow'
            # this value to be replaced later by Sarah: i replaced it by the demand so whatever is grown is in the range of demand values 
            tmp_grow = crop_demand * np.random.rand()
            ...
            if (crop_inventory < crop_demand):
                tmp_import, crop_inventory, cost, env, import_dict = import_crop(crop_demand, external_water_requirement, crop_inventory, country_facts, month, cost, env)
        ...
        return [4, tmp_grow, tmp_import, crop_inventory, cost, env, import_dict] # '4' means 'do nothing'
...
def main():
    ...
    demand = pd.read_csv('demand_dt_3.csv')
    country_facts = pd.read_csv('Country_Facts.csv')
    local_water_requirement = 60000
    crop_inventory = 0
    month = 45
    crop_demand = 1000000
