...
def import_crop(crop_demand, external_water_requirement, crop_inventory, country_facts, month, cost, env):
    ...
    for country in list(external_water_requirement_row.index):
        ...
        tmp_import_cum += tmp_import

        if (tmp_import_cum + crop_inventory) >= crop_demand:
            import_amount = tmp_import - ((tmp_import_cum + crop_inventory) - crop_demand)
        else:
            import_amount = tmp_import
        
        print("Amount imported from %s is %f" % (country, import_amount)) # logging import info
        # store import
        country_dict.update({country:import_amount})
...
def main():
    ...
    demand = pd.read_csv('demand_dt_2.csv')

    country_facts = pd.read_csv('Country_Facts.csv')
    local_water_requirement = 30000
    crop_inventory = 0
    month = 58
