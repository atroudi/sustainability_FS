...
def load_model2(month):
    ...
    # TODO: [DONE] Change the startDate offset to current date and the OFFSET limit to the prediction date
    ...
    try:
        ...
        cur.execute(
            "Select temp_avg,humidity_avg,solar_radiation,water_loss,time from geolocation_records where geolocation_id=%d and time between '%s' and '%s'" % (
                geolocation_id, current_date, prediction_date))
...
@shared_task
def decision(crop_demand, month):
    # month = 1
    print("Start decision")
    project_root = os.path.dirname(os.path.abspath(__file__))
    demand = pd.read_csv(project_root + '/demand_dt_3.csv')
    country_facts = pd.read_csv(project_root + '/Country_Facts.csv')
    local_water_requirement = load_model2(month)
